...
class SidecarEvaluatorExperimental(SidecarEvaluator):
  ...
  Please use `tf.keras.utils.SidecarEvaluator` instead.
...
class SidecarEvaluatorModelExport(ModelCheckpoint):
  ...
  Args:
      export_filepath: Path where best models should be saved by this
        ...
        as `os.path.join(best_model_dir, 'best-model-{epoch:04d}')`, can be used
        to allow saved model to preserve epoch information in the file name.
        SidecarEvaluatorModelExport will use the "training epoch" at which the
  ...
  def on_test_begin(self, logs=None):
    ...
    if most_recent_filepath is not None:
      self.export_index = (
          int(re.match(r".*ckpt-(?P<ckpt>\d+)", most_recent_filepath)["ckpt"])
          - 1
      )
    else:
      self.export_index = 0
